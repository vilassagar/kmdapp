...
import { getIncompleteTransactions } from "@/services/reports";
...
const IncompleteTransactions = () => {
  ...
  const handleChange = (name) => (event) => {
    let nextState = produce(transaction, (draft) => {
      switch (name) {
        case "association":
          draft[name] = event;
          break;
  ...
  const handleDownload = async () => {
    try {
      const postObject = getCompletedFormsPostObject(transaction);
      const response = await getIncompleteTransactions(postObject);
      ...
      link.setAttribute(
        "download",
        `report_incompleteTransactions_${String(new Date().getDate()).padStart(
          2,
          "0"
      ...
      toast({
        variant: "destructive",
        title: "Something went wrong.",
        description: "Unable to get incomplete transactions.",
      });
    }
  };
  return (
...
export default WithLayout(IncompleteTransactions);
